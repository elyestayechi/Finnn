...
export const pdfApi = {
  ...
  delete: async (reportId: number): Promise<{ message: string; id: number }> => {
    const response = await fetch(`${API_BASE_URL}/pdf-reports/${reportId}`, {
      method: 'DELETE',
    });

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.detail || 'Failed to delete PDF report');
    }
    ...
    toast({
      ...
      description: 'PDF report has been deleted successfully.',
    });
