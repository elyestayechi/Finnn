...
from analyse import load_loan_data_fallback
    ...
    async def send_message(self, analysis_id: str, message: Dict[str, Any]):
        if analysis_id in self.active_connections:
            try:
                ...
                logger.debug(f"WebSocket message sent to {analysis_id}: {message.get('type')}")
...
async def process_loan_with_websocket(data_loader, risk_engine, business_rules, llm_analyzer, analysis_id, loan_id=None, external_id=None):
    """Process loan with detailed WebSocket updates including PDF generation"""
    logger.info(f"STARTING ANALYSIS PROCESS for analysis_id: {analysis_id}, loan_id: {loan_id}, external_id: {external_id}")
    
    try:
        # Send immediate confirmation that processing has started
        await send_websocket_update(analysis_id, "log", {
            "message": "ANALYSIS PROCESS STARTED - Initializing system...",
            "level": "info"
        })
        ...
        await send_websocket_update(analysis_id, "log", {
            "message": "Starting new loan assessment session",
            "level": "info"
        })
        
        # Send initial progress
        await send_websocket_update(analysis_id, "progress", {
            "progress": 10
        })
        
        # Use the existing process_loan function from analyse.py instead of reimplementing
        await send_websocket_update(analysis_id, "log", {
            "message": "Fetching loan data (with fallback)",
            "level": "info"
        })
        ...
        raw_loan_data = await loop.run_in_executor(
            thread_pool,
            lambda: asyncio.run(load_loan_data_fallback(loan_id, external_id))
        )
        
        if not raw_loan_data:
            raise ValueError("No loan data received")
        
        await send_websocket_update(analysis_id, "log", {
            "message": "Loan data loaded successfully",
            "level": "success"
        })
        ...
        await send_websocket_update(analysis_id, "log", {
            "message": f"Risk assessment completed - Score: {assessment['risk_assessment']['total_score']}, Level: {assessment['risk_assessment']['risk_level']}",
            "level": "success"
        })
        ...
        analysis = await loop.run_in_executor(
            thread_pool,
            llm_analyzer.analyze_loan,
            assessment
        )
        assessment['llm_analysis'] = analysis
        
        await send_websocket_update(analysis_id, "log", {
            "message": f"AI Recommendation: {analysis['recommendation'].upper()}",
            "level": "success"
        })
        ...
        try:
            ...
            await send_websocket_update(analysis_id, "log", {
                "message": f"PDF report generated: {report_filename.name}",
                "level": "success"
            })
            ...
            try:
                await loop.run_in_executor(
                    thread_pool,
                    llm_analyzer.store_current_loan,
                    assessment
                )
                await send_websocket_update(analysis_id, "log", {
                    "message": "Loan data stored in vector database",
                    "level": "success"
                })
            except Exception as e:
                await send_websocket_update(analysis_id, "log", {
                    "message": f"⚠️ Failed to store loan in vector DB: {str(e)}",
                    "level": "warning"
                })
            
            # Create PDF report entry in database
            try:
                db = next(get_db())
                ...
                await send_websocket_update(analysis_id, "log", {
                    "message": "PDF report saved to database",
                    "level": "success"
                })
            except Exception as e:
                await send_websocket_update(analysis_id, "log", {
                    "message": f"⚠️ Failed to save PDF to database: {str(e)}",
                    "level": "warning"
                })
            
        except Exception as e:
            await send_websocket_update(analysis_id, "log", {
                "message": f"⚠️ PDF generation failed: {str(e)}",
                "level": "error"
            })
            raise
        
        # Send completion
        await send_websocket_update(analysis_id, "result", {
            "analysis": analysis,
            "assessment": assessment,
            "pdf_generated": True,
            "pdf_filename": report_filename.name,
        ...
        await send_websocket_update(analysis_id, "error", {
            "message": f"⚠️ Processing failed: {str(e)}",
            "level": "error"
        })
        raise

@app.post("/api/analyses/{analysis_id}/test-message")
async def test_websocket_message(analysis_id: str, message: Dict[str, Any]):
    """Test sending a WebSocket message to a specific analysis"""
    try:
        await manager.send_message(analysis_id, {
            "type": "log",
            "message": f" TEST: {message.get('text', 'Test message')}",
            "level": "info",
            "timestamp": datetime.now().isoformat()
        })
...
async def websocket_endpoint(websocket: WebSocket, analysis_id: str):
    ...
    try:
        ...
        while True:
            ...
            # Optional: handle ping messages or other client messages
            if data == "ping":
...
# ✅ UPDATED: Analysis creation endpoint with detailed WebSocket logging
@app.post("/api/analyses")
async def create_analysis(loan_data: Dict[str, Any]):
    """Create a new loan analysis with detailed WebSocket updates"""
    ...
    try:
        ...
        # ✅ CRITICAL FIX: Start analysis in background task WITH proper async handling
        asyncio.create_task(
            process_loan_with_websocket(
                data_loader, risk_engine, business_rules, llm_analyzer,
                analysis_id, 
                loan_data.get('loan_id'), 
                loan_data.get('external_id')
            )
        )
        
        # ✅ Send immediate response so frontend can connect WebSocket
...
@app.delete("/pdf-reports/{report_id}")
def delete_pdf_report(report_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"Deleted PDF report: {report_id}")
        
        return {"message": "PDF report deleted successfully", "id": report_id}
        
    except Exception as e:
        db.rollback()
        logger.error(f"Error deleting PDF report {report_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to delete PDF report")        
